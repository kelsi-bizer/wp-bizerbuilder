<?php
/**
 * WP-Bizerbuilder AJAX Theme Extender class.
 *
...
class Theme_Extender {
	...
	public function extract_theme_hooks() {
		...
		if ( empty( $theme_slug ) ) {
			wp_send_json_error( esc_html__( 'No theme slug specified.', 'wp-bizerbuilder' ) );
		}

		$hooks = \WP_Bizerbuilder\Hooks_Extender::get_theme_hooks( $theme_slug );
	...
	public function generate_extend_theme_plan() {
		...
		$hooks      = \WP_Bizerbuilder\Hooks_Extender::get_theme_hooks( $theme_slug );
		if ( empty( $hooks ) ) {
			wp_send_json_error( esc_html__( 'No hooks found in the theme.', 'wp-bizerbuilder' ) );
		}
	...
	public function generate_extend_theme_file() {
		...
		$all_hooks      = \WP_Bizerbuilder\Hooks_Extender::get_theme_hooks( $theme_slug );
		$selected_hooks = [];
