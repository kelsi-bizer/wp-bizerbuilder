<?php
/**
 * WP-Bizerbuilder AJAX Hooks Extender class.
 *
...
class Hooks_Extender {
	...
	public function generate_extend_hooks_plan() {
		...
		check_ajax_referer( 'wp_bizerbuilder_generate', 'security' );

		if ( ! isset( $_POST['plugin_file'] ) || ! isset( $_POST['plugin_issue'] ) ) {
			wp_send_json_error( esc_html__( 'Missing required parameters.', 'wp-bizerbuilder' ) );
		}

		$plugin_file = sanitize_text_field( wp_unslash( $_POST['plugin_file'] ) );
		$hooks       = \WP_Bizerbuilder\Hooks_Extender::get_plugin_hooks( $plugin_file );
		if ( empty( $hooks ) ) {
			wp_send_json_error( esc_html__( 'No hooks found in the plugin.', 'wp-bizerbuilder' ) );
		}
		...
		$extender       = new \WP_Bizerbuilder\Hooks_Extender( $planner_api );
		$prompt_images  = isset( $_POST['prompt_images'] ) ? \WP_Bizerbuilder\AI_Utils::parse_prompt_images( $_POST['prompt_images'] ) : [];
		...
		if ( ! $plan_array ) {
			wp_send_json_error( esc_html__( 'Failed to decode the generated plan.', 'wp-bizerbuilder' ) );
		}
	...
	public function generate_extend_hooks_code() {
		...
		$extender  = new \WP_Bizerbuilder\Hooks_Extender( $coder_api );
	...
	public function generate_extend_hooks_file() {
		...
		$all_hooks      = \WP_Bizerbuilder\Hooks_Extender::get_plugin_hooks( $plugin_file );
		$selected_hooks = [];
	...
	public function extract_hooks() {
		...
		if ( empty( $plugin_file ) ) {
			wp_send_json_error( esc_html__( 'No plugin file specified.', 'wp-bizerbuilder' ) );
		}
